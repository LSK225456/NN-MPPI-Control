...
def create_dynamics_model(config: dict, num_particles: int) -> DynamicsModel:
    """
    工厂函数：根据配置创建动力学模型
    
    Args:
        config: 配置字典，包含:
            - use_neural: bool, 是否使用神经网络
            - backend: str, 后端选择 ("pytorch" 或 "keras")，默认 "pytorch"
            - model_path: str, Keras 模型路径 (.h5)
            - pytorch_model_path: str, PyTorch 模型路径 (.pt)
            - scaler_x_path: str, 输入标准化器路径
            - scaler_y_path: str, 输出标准化器路径
            - device: str, PyTorch 设备 ("cpu" 或 "cuda")
    ...
    Example:
        ...
        ...     'use_neural': True,
        ...     'backend': 'pytorch',
        ...     'pytorch_model_path': '/path/to/model.pt',
        ...     'scaler_x_path': '/path/to/scaler.pkl',
        ...     'scaler_y_path': '/path/to/scaler_y.joblib',
        ...     'device': 'cpu'
        ... }
        >>> model = create_dynamics_model(config, num_particles=500)
    """
    # 不使用神经网络时，返回运动学模型
    ...
    if backend == 'pytorch':
        try:
            ...
            print("[DynamicsFactory] 尝试回退到 Keras 后端...")
            backend = 'keras'  # 回退到 Keras
    ...
    if backend == 'keras':
        try:
            ...
            print("[DynamicsFactory] 回退到运动学模型")
            return KinematicDynamics()
