...
class FG_eval 
{
    public:
        ...
        { 
            ...
            _cte_int_start = _etheta_start + _mpc_steps;    //1026:定义新的状态索引

            _angvel_start = _cte_int_start + _mpc_steps;    //1026
        ...
        void LoadParams(const std::map<string, double> &params)
        {
            ...
            _cte_start   = _v_start + _mpc_steps;
            _etheta_start  = _cte_start + _mpc_steps;
            _cte_int_start = _etheta_start + _mpc_steps;        //1026
        ...
        void operator()(ADvector& fg, const ADvector& vars) 
        {
            ...
            for (int i = 0; i < _mpc_steps; i++) 
            {
              ...
              fg[0] += _w_cte_int * CppAD::pow(vars[_cte_int_start + i], 2);    //1026
            ...
            fg[1 + _cte_int_start] = vars[_cte_int_start];  //1026:设置积分项的初始约束
            ...
            for (int i = 0; i < _mpc_steps - 1; i++)
            {
                ...
                AD<double> cte_int1 = vars[_cte_int_start + i + 1];     //1026:获取 t+1 时刻的积分项
                ...
                AD<double> cte_int0 = vars[_cte_int_start + i];     //1026:获取 t 时刻的积分项
                ...
                fg[2 + _cte_int_start + i] = cte_int1 - (cte_int0 + cte0 * _dt);    //1026:定义积分项的状态转移方程
            }
        }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) 
{
    ...
    const double cte_int = state[6];    //1026:解包新的积分状态

    // Set the number of model variables (includes both states and inputs).
    // For example: If the state is a 4 element vector, the actuators is a 2
    // element vector and there are 10 timesteps. The number of variables is:
    // 4 * 10 + 2 * 9
    size_t n_vars = _mpc_steps * 7 + (_mpc_steps - 1) * 2;  //1026:更新变量和约束的总数
    
    // Set the number of constraints
    size_t n_constraints = _mpc_steps * 7;      //1026:更新变量和约束的总数

    // Initial value of the independent variables. 创建变量
    ...
    // Set the initial variable values 初始化变量值
    vars[_x_start] = x;
    vars[_y_start] = y;
    ...
    vars[_cte_int_start] = cte_int;     //1026: 设置积分项的初始值
    ...
    constraints_lowerbound[_cte_int_start] = cte_int;   //1026:设置积分项的初始约束
    ...
    constraints_upperbound[_cte_int_start] = cte_int;   //1026
