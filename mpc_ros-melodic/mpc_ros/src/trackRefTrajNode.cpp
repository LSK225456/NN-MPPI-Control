...
class MPCNode
{
    public:
        ...
        void dynamic_reconfigure_Callback(mpc_ros::MPCPlannerConfig &config);

    dynamic_reconfigure::Server<mpc_ros::MPCPlannerConfig> server;

    dynamic_reconfigure::Server<mpc_ros::MPCPlannerConfig>::CallbackType f;
...
MPCNode::MPCNode()
{
    ...
    cout<< "_max_speed" << _max_speed  << endl;
    ...
    f = boost::bind(&MPCNode::dynamic_reconfigure_Callback, this, _1);
    server.setCallback(f);
    cout<< "1111" << endl;
}

void MPCNode::dynamic_reconfigure_Callback(mpc_ros::MPCPlannerConfig &config)
{
...
_bound_value = config.mpc_bound_value;
    ...
    // ROS_INFO("_debug_info _delay_mode _max_speed = [%f %f %f]",_debug_info,_delay_mode,_max_speed);
}
    ...
    try
    {
        ...
        for(int i = min_idx; i < N ; i++)
        {
            ...
            _tf_listener.transformPose("world", ros::Time(0) , 
                                            totalPathMsg->poses[i], "world", tempPose);                     
